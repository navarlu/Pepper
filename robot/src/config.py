...
# Runtime state shared with the services (`experiment_active`, volume).
STATE_FILE = _env_str(
    "STATE_FILE",
    os.path.join(REPO_ROOT, "services", "data", "state.json"),
)

# Pepper audio stream rate (reported by the bridge).
...
# Pepper speaker volume.
PEPPER_OUTPUT_VOLUME = _env_int("PEPPER_OUTPUT_VOLUME", 100)
