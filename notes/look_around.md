...
**Current state (2026-04-16):** infrastructure is all built and verified end-to-end. The tool is **gated off** (`ENABLE_LOOK_AROUND_TOOL = False` in [config.py](../voice-agent/src/config.py:89)) because of a regression with the local Qwen-7B-Instruct model when 3 tools are exposed. The 2-tool baseline (`query_search` + `play_animation`) is restored to the known-good state from commit `08d8ddd`. Flip the flag to re-enable; everything else still wired and running.
...
- **Also:** hit a dumb name mismatch — `LOCAL_LLM_MODEL` in [config.py](../voice-agent/src/config.py) was pointing at `Qwen/Qwen2.5-VL-7B-Instruct` while vLLM was serving the 3B → every request returned `404 NotFoundError`. Fix: keep the model name in config in sync with what vLLM actually serves.
...
- [voice-agent/src/tools.py](../voice-agent/src/tools.py) — `_fetch_camera_snapshot()`, `_describe_image_with_vl()`, `look_around` tool (inside `build_tools()`), gated by `ENABLE_LOOK_AROUND_TOOL`.
- [voice-agent/src/config.py](../voice-agent/src/config.py) — `ENABLE_LOOK_AROUND_TOOL` flag, `LOOK_AROUND_*` knobs, conditional system-prompt blocks.
