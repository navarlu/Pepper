...
As noted in the motivation (Section~\ref{sec:motivation}), two pre-LLM deployments anchor this work: Valerie the Roboceptionist at Carnegie Mellon University (2005)~\cite{gockley2005longterm} and Robovie in a Japanese shopping mall (2010)~\cite{kanda2010shoppingmall}.
...
The dialogue was authored in advance, and the team explicitly identifies the resulting brittleness as a long-term design problem: Valerie would continue telling her stories after visitors had departed, the interaction style did not adapt for returning visitors, and any topic outside the scripted set had no graceful answer.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{src/figures/related_work/shoping_mall.png}
    \caption{Robovie deployed in a Japanese shopping mall, giving route guidance to visitors~\cite{kanda2010shoppingmall}.}
    \label{fig:robovie}
\end{figure}
...
\etal{Chen} (2024)~\cite{chen2024relatable} replaced Pepper's limited built-in keyword speech recognition with Whisper~\cite{radford2023whisper} for transcription and routed the user's words through ChatGPT for the reply, with the explicit aim of testing whether a more capable language layer makes the robot feel more relatable.
...
\etal{Herath} (2025)~\cite{herath2025firstimpressions} deployed a comparable Pepper--ChatGPT system at a public science festival and gathered first impressions from the visitors.
...
A more directly comparable system is the one by \etal{Qu} (2024)~\cite{qu2024robot}, who built a ChatGPT-based control platform for multimodal interaction with humans on a humanoid robot.
They wired the LLM into the dialogue loop alongside speech recognition and synthesis, and reported that the resulting interaction felt more natural than a scripted baseline.

The closest sibling system in the literature is FurChat by \etal{Cherakara} (2023)~\cite{cherakara2023furchat}, a receptionist robot at the National Robotarium.
They deployed GPT-3.5 on a Furhat robot and combined an open-domain dialogue mode with a closed-domain mode backed by a small dictionary scraped from the host institution's website.
The LLM also picked an emoticon for each reply, which was mapped to one of Furhat's pre-defined facial gestures.
FurChat was presented as a demonstrator rather than evaluated with users, but it showed that a single LLM could handle both factual questions about a venue and casual smalltalk, with the embodiment layer (facial expressions) generated by the same reasoning step.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{src/figures/related_work/furchat.png}
    \caption{FurChat on the Furhat robot, with an LLM-selected facial expression accompanying the reply~\cite{cherakara2023furchat}.}
    \label{fig:furchat}
\end{figure}


The idea of an LLM controlling a robot through a small set of tools (see Section~\ref{sec:tool_calling}) predates these social robotics applications.
\etal{Huang} (2022)~\cite{huang2022inner} introduced an ``inner monologue'' loop in which the language model alternates between planning the next step and reading feedback from the robot's perception and skills, and \etal{Ahn} (2022)~\cite{ahn2022saycan} filtered LLM action proposals through learned estimates of which actions the robot could actually carry out on a mobile manipulator.
...
\etal{Saunderson} (2019)~\cite{saunderson2019survey} survey this literature and document robust effects on engagement, likeability, perceived intelligence, and trust across many platforms and tasks.

A more targeted question---whether the gestures need to match the speech exactly to be useful---is addressed by \etal{Salem} (2013)~\cite{salem2013err} in a controlled study.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{src/figures/related_work/salem.png}
    \caption{The humanoid robot used in the Salem et al.\ study comparing speech-only, congruent-gesture, and incongruent-gesture conditions~\cite{salem2013err}.}
    \label{fig:salem}
\end{figure}


\section{Evaluation of Social Robotics Interaction}
...
The most widely used instrument is the Godspeed questionnaire of \etal{Bartneck} (2009)~\cite{bartneck2009godspeed}, which measures five separate attributes on bipolar adjective scales: anthropomorphism (humanlike versus machinelike), animacy (how lifelike), likeability (how friendly), perceived intelligence, and perceived safety.
...
\etal{Carpinella} (2017)~\cite{carpinella2017rosas} later proposed the Robotic Social Attributes Scale (RoSAS) as a complement to Godspeed, reorganizing the social attribution literature into three factors: warmth, competence, and discomfort.
...
A different angle on evaluation is taken by the Almere model of \etal{Heerink} (2010)~\cite{heerink2010almere}, which extends the Technology Acceptance Model to assistive social robots and asks not what the robot is perceived to be, but whether the user intends to use it again.
