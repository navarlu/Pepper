"""Dispatcher — keeps exactly one agent dispatched into
the fixed experiment room.

Why this exists: each worker registers with an explicit `agent_name`,
which means LiveKit only sends it a job when someone creates an *agent
dispatch* for the room. `docker compose up` should be the only manual
step, so this small loop does it — and re-does it whenever the agent's job ends
...
Run: uv run python services/src/dispatcher.py
"""
...
async def _ensure_dispatched(lkapi: api.LiveKitAPI) -> bool:
    ...
    try:
        ...
        print(
            f"[dispatcher] room {ROOM_NAME!r} not listable yet "
            f"({exc}) — waiting",
            flush=True,
        )
        return False
    ...
    try:
        ...
        for d in stale:
            print(
                f"[dispatcher] deleting stale dispatch id={d.id} "
            ...
            try:
                ...
                print(
                    f"[dispatcher] delete_dispatch id={d.id} "
                    f"failed err={exc}",
                    flush=True,
                )
    except Exception as exc:
        print(f"[dispatcher] list_dispatch failed err={exc}", flush=True)
    ...
    try:
        ...
        print(
            f"[dispatcher] created dispatch id={dispatch.id} "
        ...
        print(
            f"[dispatcher] create_dispatch failed err={exc} — "
            "will retry",
            flush=True,
        )
        return False
...
async def main() -> None:
    ...
    print(
        f"[dispatcher] started room={ROOM_NAME} agent={AGENT_NAME} "
