...
Run order for a study session:
    ...
    #   python voice-agent/tests/local_llm_benchmark/experiment_worker.py dev
    ...
    try:
        ...
        if not agents:
            print("[experiment] ERROR: no agent participant joined the "
                  "room within 60s. Is the experiment_worker.py running "
        ...
        if len(agents) > 1:
            print(f"[experiment] ERROR: {len(agents)} agent participants "
                  ...
                  f"    pkill -f experiment_worker.py\n"
