...
What's different from production agent.py:
  - Tools come from `tools_toolonly.py` (Letta-style: send_message_to_user
...
Run:
    uv run python voice-agent/tests/local_llm_benchmark/experiment_worker.py dev
    # or, with explicit agent name (must match dispatch agent_name):
    uv run python voice-agent/tests/local_llm_benchmark/experiment_worker.py dev --agent-name pepper-experiment
...
from src.local_speech import FasterWhisperSTT, PiperTTS  # noqa: E402
from src.qwen_compat import install_function_args_patch  # noqa: E402

import tools_toolonly  # noqa: E402
from src.bridge_client import post_tablet_clear  # noqa: E402
from tools_toolonly import LIVEKIT_TOOLS_TOOLONLY  # noqa: E402

logger = logging.getLogger("experiment-worker")

# Quiet livekit's INFO/DEBUG noise except for our own logger.
...
# ── System prompt (mirrors livekit_console_toolonly.py) ──────────────
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    # Wire tool events from tools_toolonly into the publish channel.
    ...
    tools_toolonly.set_tool_event_listener(_on_tool_event)
    tools_toolonly.set_tool_result_listener(_on_tool_result)
