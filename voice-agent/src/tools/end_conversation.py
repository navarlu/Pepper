...
machinery down to what the streaming workers actually need:
  ...
  2. Publish `farewell_active=True` on `pepper.state` so tablet-server
     ...
     is ever published — see `session_state.publish_state`.)
  ...
  4. Speak a short farewell + the fixed conversation-ID reminder via
     ...
     wrapping needed.
  5. Call `session_state.request_end_session()` which flips the
...
from bridge_client import post_tablet_farewell  # noqa: E402
...
# Worker-shared session state lives at the top of src/.
...
async def end_conversation(
    ...
    student_id = session_state.get_student_id()
    ...
    await session_state.publish_state({"farewell_active": True})
    ...
    await session_state.request_end_session("end_conversation_tool")
