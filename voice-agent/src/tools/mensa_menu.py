...
from tools.utils.mensa import fetch_mensa_menu  # noqa: E402
...
async def mensa_menu(
    ...
    Call this when the user asks what's for lunch, what they can
    eat, what's on the menu, or about the canteen / mensa / menza /
    buffet.
    ...
    result["instruction"] = (
        "Match the user's question to the right day in `days`. "
        "Mention only 1 or 2 dishes — prefer main dishes over soups "
        "unless the user asked specifically. If the user asked for a "
        "specific category (soup, vegetarian, etc.), filter `dishes` "
        "by that category. Read the FULL list ONLY if the user "
        "explicitly asked for everything. Never read out the canteen's "
        "full name or any URLs."
    )
