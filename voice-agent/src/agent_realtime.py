"""Realtime (speech-to-speech) worker (compose service `agent-realtime`).
...
`agent.py`; the audio plumbing around it
is unchanged. Differences vs. the cascade worker:
  ...
  * Model flips between the study's two conditions via the
    `REALTIME_MODEL` env var — no A/B loop, no launcher.
  * Dispatch: the worker registers under `AGENT_NAME`;
...
Run (worker, inside the compose):
    uv run python voice-agent/src/agent_realtime.py start

Quick local test with laptop mic+speakers (no LiveKit room needed):
    uv run python voice-agent/src/agent_realtime.py console
"""
...
# ── Env setup ─────────────────────────────────────────────────
# Run as `python voice-agent/src/agent_realtime.py`, so src/ is on
# sys.path and `tools`, `bridge_client` … import as top-level modules.
...
from prompt_realtime import SYSTEM_PROMPT  # noqa: E402
from tools.realtime.find_room import find_room  # noqa: E402
from tools.realtime.lookup_person import lookup_person  # noqa: E402
from tools.utils._events import (  # noqa: E402
    set_tool_event_listener,
    set_tool_result_listener,
)
from bridge_client import post_head_lock  # noqa: E402

REALTIME_TOOLS = [find_room, lookup_person]
...
# ── Tunables ─────────────────────────────────────────────────────────
AGENT_NAME = os.environ.get("AGENT_NAME", "pepper-agent-realtime")
...
print(
    f"[agent-realtime] booted, waiting for dispatch "
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        print(
            "[agent-realtime] ERROR: OPENAI_API_KEY not set "
    ...
    async def _log_usage_summary():
        try:
            ...
            print(f"[agent-realtime] usage_summary={summary}", flush=True)
        except Exception as exc:
    ...
    agent = Agent(instructions=SYSTEM_PROMPT, tools=REALTIME_TOOLS)
    start_kwargs: dict = {}
    if not IS_CONSOLE:
    ...
    print(
        f"[agent-realtime] session_started model={REALTIME_MODEL} "
    ...
    print(
        f"[agent-realtime] === REALTIME CONTEXT DUMP ===\n"
        f"--- MODEL {REALTIME_MODEL} voice={REALTIME_VOICE} lang={LANG} ---\n"
        f"--- SYSTEM_PROMPT ({len(SYSTEM_PROMPT)} chars) ---\n"
        f"{SYSTEM_PROMPT}\n"
        f"--- TOOLS ({len(REALTIME_TOOLS)}) ---\n"
        f"{_format_tool_summary(REALTIME_TOOLS)}\n"
        f"=== END DUMP ===",
        flush=True,
    )
    ...
    ctx.shutdown(reason="agent_realtime_done")
