"""Cascaded streaming worker (compose service `agent`).
...
few hundred ms of TTS buffer.
  ...
  * No mic-mute state machine: self-echo is handled by WebRTC AEC3 in
    `services/src/user_client.py`, so the mic stays open.
...
Run (worker):
    uv run python voice-agent/src/agent.py dev
"""
...
# ── Env setup ─────────────────────────────────────────────────
# Run as `python voice-agent/src/agent.py`, so src/ is on sys.path and
# `config`, `tools`, `bridge_client` … import as top-level modules.
...
# ── Prompt + tools ───────────────────────────────────────────────────
from prompt import SYSTEM_PROMPT, GREETING_INSTRUCTIONS  # noqa: E402
...
from tools.end_conversation import end_conversation  # noqa: E402
...
from config import ENABLE_INLINE_GESTURES  # noqa: E402
...
# ── Tunables ─────────────────────────────────────────────────────────
AGENT_NAME = os.environ.get("AGENT_NAME", "pepper-agent")
...
print(
    f"[agent] booted, waiting for dispatch "
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        print(
            "[agent] ERROR: OPENAI_API_KEY not set "
    ...
    async def _log_usage_summary():
        try:
            ...
            print(f"[agent] usage_summary={summary}", flush=True)
        except Exception as exc:
    ...
    # Tools (specifically `end_conversation`) call this to
    ...
    session_state.set_end_session_callback(_end_session_from_tool)
    ...
    ctx.shutdown(reason="agent_done")
